...
class ExcelGenerator:
    ...
    def generate_excel_files(cls, data, date_obj):
        ...
        for attempt in range(cls.MAX_RETRIES):
            try:
                if cls.is_file_locked(excel_filename) or cls.is_file_locked(summary_filename):
                    print(f"Files locked, attempt {attempt + 1} of {cls.MAX_RETRIES}")
                    # Try to use COM to save changes
                    try:
                        pythoncom.CoInitialize()
                        excel = win32com.client.Dispatch("Excel.Application")
                        for wb in excel.Workbooks:
                            if wb.FullName == excel_filename or wb.FullName == summary_filename:
                                wb.Save()
                                print(f"Saved changes to open file: {wb.FullName}")
                        excel.Quit()
                    except:
                        if attempt < cls.MAX_RETRIES - 1:
                            time.sleep(cls.RETRY_DELAY)
                            continue
                        raise PermissionError("Excel files are in use and maximum retries reached")
                ...
                if attempt == cls.MAX_RETRIES - 1:
                    raise Exception("Excel files are in use. Please close them and try again.")
                time.sleep(cls.RETRY_DELAY)
