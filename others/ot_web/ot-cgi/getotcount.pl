...
#Perl wday starts on Sunday. Ex. 0 = Sunday, 3 = Wednesday
#Date add and subtract is by seconds, ONE_DAY is equivalent to 86400 seconds or 1 day.
my $appDate = Time::Piece -> strptime("$jData->{ApplyDate}","%Y%m%d");
my $appID = $jData->{id};
my $sDate = $appDate->subtract(ONE_DAY * (($appDate->wday - 2)));
my $eDate = $sDate->add(ONE_DAY * 6);
if ($appDate->wday == 1) {
	$sDate = $appDate->subtract(ONE_DAY * 7);
	$eDate = $appDate;
}
	...
	for $i (@monthRange){
		...
		if (-e $filename) {
			...
			for $a( @{$otData->{PTBOTList}} ) {
				...
				if (($sDate->epoch <= $applyDate->epoch)
					&&($eDate->epoch >= $applyDate->epoch)
					&&($a->{ApplicantID} eq $jData->{ApplicantID})
					&&($appDate->epoch != $applyDate->epoch)
					&&($appID != $a->{id})) {
					$otCount += $a->{ActualOTHrs};
				}
			}
		}
	}
